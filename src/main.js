...
const MISC = window.MISC = {
    ...
    newElem: (elem) => {
        let nElem = document.createElement(elem.type);
        nElem.id = elem.id || "";
    ...
    updateCls: (trgtElem, args) => {
        let addCls = (trgt, item) => !trgt.classList.contains(item.slice(1)) ? trgt.classList.add(item.slice(1)) : null;
        let subCls = (trgt, item) => trgt.classList.contains(item.slice(1)) ? trgt.classList.remove(item.slice(1)) : null;
        let verCls = (trgt, item) => {
            !item.includes(" ") ?
                item.charAt(0) == "-" ?
                    subCls(trgt, item)
                    : item.charAt(0) == "+" ?
            ...
            if (typeof (events.custom) == "object" && Object.entries(events.custom).length != 0) {
                Object.entries(events.custom).forEach(([key, value]) => {
                    if (typeof value == "function") {
                        ...
                        MISC.throwError("custom func on setupEvent must be a function");
                });
            }
        }
        else
            MISC.throwError("setupEvents' 'elem' && 'events' must be both 'object's");
    },
    useDataImage: (image) => {
        if (typeof (dataImage[image]) != "undefined") {
            ...
            MISC.throwError("dataImage." + image + " is missing");
    },
    waitFor: (dur) => {
        return new Promise((resolve) => {
    ...
    replaceIfNull: (trgt, replacement) => {
    ...
    getSVG: (name) => {
        if (typeof (dataImage[name]) != "undefined") {
            ...
            if (typeof trgtSvg.src != "undefined") {
                let svg = MISC.getSVG(trgtSvg.src);
                if (typeof trgtSvg.styles != "undefined") {
                    trgtSvg.styles.forEach((style) => {
                        MISC.setStyle(svg, {
                    ...
                    MISC.throwError("dataImage['" + name + "'].styles is undefined");
                }
            }
            else {
            ...
            MISC.throwError("dataImage['" + name + "'] is undefined");
    },
    setSVGStyle: (elem, styleName, styleVal) => {
...
let loaderCont, hintCont, uiCont, inspectCont, panoCont, mdlCont, carouselCont, slideCont, docCont, interactiveVidCont;
(() => {
...
(() => {
        ...
        let uiCont = MISC.newElem({
            ...
            setup: () => {
                ...
                let btn = uiCont.btn = MISC.newElem({
                    type: "button",
                    events: obj.events,
                    props: Object.assign(Object.assign({}, obj.props || {}), {
                        toggle: (() => {
                            if (obj.toggle) {
                                return 0;
                            }
    ...
    let genTooltip = (obj) => {
        obj.elem.tooltip = (0, tippy_js_1.default)(obj.elem, {
            placement: obj.placement || "top",
            content: obj.name,
            touch: false,
            // theme:"translucent"
    ...
    let genTimeline = (obj) => {
        ...
        if (typeof obj.events == "object") {
            MISC.setupEvents(uiCont, obj.events);
        }
        //Timeline
        ...
        (() => {
            uiTimeline.tooltip = (0, tippy_js_1.default)(uiTimeline, {
                placement: "top",
                hideOnClick: false,
                content: "NaN%",
                touch: false,
            ...
            MISC.setupEvents(uiTimeline, {
                ...
                custom: {
                    pointermove: (event, media = playerBar.videoBar.currentMedia) => {
    ...
    let genTriangularVolume = (obj) => {
        ...
        let vlmBg = MISC.newElem({
            type: "div",
            identity: ".ui-trivolume-bg"
        });
        let vlmTxt = MISC.newElem({
            type: "span",
            identity: ".ui-trivolume-text",
            innerText: "Vol"
        });
        let vlmSlider = uiCont.volume = MISC.newElem({
            type: "input",
        ...
        (() => {
            ...
            vlmSlider.tooltip = (0, tippy_js_1.default)(vlmSlider, {
                placement: "top",
                hideOnClick: false,
                content: "100%",
                touch: false,
    ...
    let genPlayerVideoOptions = (obj) => {
        ...
        //     }
        // })
        let optionMain4 = genPlayerOption({
    ...
    let genPlayerViewerOptions = (obj) => {
        let gear = genUI(obj);
        //Main options
    ...
    (() => {
        ...
        let uiVisibilityManager = window.uiVisibilityManager = () => {
    ...
    let settingBar = window.settingBar = MISC.newElem({
        ...
        props: {
            expand: (event) => {
                ...
                MISC.qsa(".secondary", settingBar).forEach((elem) => {
                    elem.showState();
                });
            ...
            shrink: (event) => {
                ...
                MISC.qsa(".secondary", settingBar).forEach((elem) => __awaiter(void 0, void 0, void 0, function* () {
                    elem.hideState();
                }));
        ...
        setup: () => {
            ...
            */
            settingBar.fullscreenUI = genUI({
                ...
                events: Object.assign(Object.assign({}, settingBar.visToggle), {
                    onActive: () => {
                        let btn = settingBar.fullscreenUI.btn;
                        if (btn.toggle == 0) {
                            btn.enterFullscreen();
    ...
    let sideBar = window.sideBar = MISC.newElem({
        ...
        props: {
            unminify: (event) => {
                ...
                //show all secondary
                MISC.qsa(".secondary", sideBar).forEach((elem) => {
                    elem.showState();
                });
                if (MISC.isKeyEvent(event)) {
        ...
        setup: () => {
            ...
            sideBar.moreUI = genUI({
                ...
                placement: "right",
                tooltip: true,
                events: Object.assign(Object.assign({}, sideBar.visToggle), { onActive: (event) => {
                        sideBar.unminify(event);
                    } }),
                appendTo: sideBar
            });
        ...
        */
        let vidFiles = mediaCont.vidFiles = window.videoFile = MISC.newElem({
            ...
            props: {
                activeVideos: [],
                resize: () => {
                    vidFiles.activeVideos.forEach((item) => {
        ...
        (() => {
            ...
            (() => {
                ...
                */
                ...
                audDisplay.drawline = (bar, height, width) => {
    ...
    (() => {
        ...
        playerBar.show = (num, expand) => {
            ...
            if (num == 0) { //video
                ...
                MISC.updateCls(viewerBar, "-hidden");
                if (expand) {
                    viewerBar.lessUI.btn.focus();
                }
            }
            else if (num == 2) { //doc
                MISC.updateCls(docBar, "-hidden");
                if (expand) {
                    docBar.lessUI.btn.focus();
                }
            }
            else if (num == 3) { //model
                MISC.updateCls(modelBar, "-hidden");
                if (expand) {
                    modelBar.lessUI.btn.focus();
                }
            }
        };
        playerBar.minify = () => {
        ...
        (() => {
            videoBar.set = (elem) => {
                ...
                if (elem.readyState == 4) {
                    ...
                    if (!MISC.isEmpty(elem.subSrc)) {
                        if (elem.subReady == false) {
                            fetch(elem.subSrc).then(r => r.text())
                             ...
                             a.split("\n\n").map((item) => {
                                ...
                                let tempSort = videoBar.currentMedia.subSort = {};
                        ...
                        playerBar.videoBar.captionUI.disable();
                    videoBar.addTimeUpdateEvent();
                }
                else
                    MISC.throwError("target media is not yet ready");
            };
            videoBar.play = (media = videoBar.currentMedia) => {
                ...
                if (!MISC.isEmpty(videoBar.currentMedia)) {
                    ...
                    MISC.throwError("videoBar.currentMedia is empty");
            };
            videoBar.addTimeUpdateEvent = () => {
                if (videoBar.currentMedia.timeEventAdded != true) {
                    let spawnObj = (event, item) => {
                    ...
                    videoBar.currentMedia.addEventListener("timeupdate", (event) => {
            ...
            videoBar.showSettings = () => {
                ...
                MISC.qsa(".ui-opt", settingUI.main).forEach((elem) => {
...
(() => {
    ...
    var pdfDoc = null, pageNum = 1, pageRendering = false, pageNumPending = null, scale = 1, canvas = docCanvas, ctx = canvas.getContext('2d');
